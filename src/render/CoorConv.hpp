...
double ArcLengthOfMeridian(double phi) {
    ...
    result = alpha * (phi + (beta * sin(2.0 * phi)) + (gamma * sin(4.0 * phi)) + (delta * sin(6.0 * phi)) +
                      (epsilon * sin(8.0 * phi)));
...
double FootpointLatitude(double y) {
    ...
    result = y_ + (beta_ * sin(2.0 * y_)) + (gamma_ * sin(4.0 * y_)) + (delta_ * sin(6.0 * y_)) +
             (epsilon_ * sin(8.0 * y_));
...
void MapXYToLatLon(double x, double y, double lambda0, WGS84Corr &philambda) {
    ...
    /* Calculate longitude */
    philambda.log = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) + x5frac * x5poly * pow(x, 5.0) +
